...
S SIX, SEVEN, FIVE, g1, gw, gs, alphaO01, alphaO02, alphaR01, alphaO03, alphaO04, alphaO05, alphaO06, alphaO07, alphaO08, alphaO09, alphaO10, alphaO11, alphaR02, alphaR03, alphaR04, alphaO12, alphaO13, alphaO14, alphaO15, alphaR05, alphaR06, alphaR07, alphaR01bar;
CF Ta, FSU2L, T, FSU3C, fsu2, fsu3;
CF Evect;
...
repeat;
id,once gam(y1?,?a, y?)*gam(y?, ?b)= gam(y1,?a, ?b);
endrepeat;
...
#do i=1,4
if (count(sfound,1) == 0);
id,once Evect(p'i',mu?)*gam(y?,?a,mu?,?b, y?) = sfound * Evect(p'i',mu)*gam(y,mu,?b, ?a,y);
endif;
#enddo
.sort
...
* if (count(sfound,1) == 0);
* repeat;
* id,once Evect(p'i',mu?)*gam(y1?,mu?,?a, y?)*gam(y?, ?b)= sfound * Evect(p'i',mu)*gam(y1,mu,?a, ?b);
* endrepeat;
* endif;
* #enddo
* .sort

* *: if none of the first four particles are vector bosons, we do it at random
* if (count(sfound,1) == 0);
* repeat;
* id,once gam(y1?,?a, y?)*gam(y?, ?b)= gam(y1,?a, ?b);
* endrepeat;
...
id gam(y1?,FIVE,?a,mua?,mua?,?b,y1?) = gam(y1,FIVE, ?a, ?b, y1) * (D + sxa * (D-4)) ;
*<<<<<<<<
...
*: if, in the end, we only need one coefficient per diagram we can remove this and add set sxa->samb('i') above.
if ( count(sxa,1) > 0 );
*: we don't care how many contractions of gamma matrices happened; sxa^n = sxa
id sxa^s? = sxa;
*: increment the dollar  variable that counts interesting traces
$counter = $counter+1;
id sxa = samb('i',$counter);
endif;
.sort
